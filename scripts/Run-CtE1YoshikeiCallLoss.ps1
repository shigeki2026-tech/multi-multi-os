...
<#
.SYNOPSIS
    ヨシケイ4スキルグループの CT-e1 呼損チェックを、ローカル data/ct_e1 運用で実行するラッパー。

.DESCRIPTION
    外部CT-e1 RPAが保存した CSV を data/ct_e1/inbox から処理し、
    data/ct_e1/outbox へ通知文プレビューとJSONサマリを出力する。
    処理成功後、元CSVは data/ct_e1/processed へ移動する。
...
if ($csvFiles.Count -eq 0) {
  $noCsvLog = [ordered]@{
    source = "wrapper"
    status = "no_csv"
    message = "No CSV files found in local inbox. Treated as normal no-op."
    inbox = $inbox
